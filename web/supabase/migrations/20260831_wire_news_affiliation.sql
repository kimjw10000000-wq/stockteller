-- 6-K Exhibit 99.1 summaries stored as news vs SEC cards on wire_news

alter table public.wire_news
  add column if not exists affiliation text;

update public.wire_news
  set affiliation = 'news'
  where affiliation is null or btrim(affiliation) = '';

alter table public.wire_news
  alter column affiliation set default 'news';
...
begin
  alter table public.wire_news
    add constraint wire_news_affiliation_check
    check (affiliation in ('news', 'sec'));
exception
  when duplicate_object then null;
end $$;

alter table public.wire_news
  add column if not exists newswire text;
...
create index if not exists wire_news_affiliation_published_idx
  on public.wire_news (affiliation, published_at desc nulls last);

create index if not exists wire_news_accession_idx
  on public.wire_news (accession);

comment on column public.wire_news.affiliation is
  'news = newswire press release; sec = SEC 6-K (or combined) summary';
comment on column public.wire_news.newswire is
  'Detected wire (GlobeNewswire, Business Wire, PR Newswire, …) when affiliation=news';
comment on column public.wire_news.form_type is
  'SEC form when sourced from EDGAR (6-K, 6-K/A)';
comment on column public.wire_news.accession is
  'EDGAR accession number when sourced from a filing';

-- Worldwide city names for Exhibit 99.1 dateline detection (city + "wire")
create table if not exists public.world_cities (
  ascii_name text primary key,
  name text,
  population integer
);

create index if not exists world_cities_name_idx
  on public.world_cities (name);

comment on table public.world_cities is
  'GeoNames cities (pop >= 15k). Used to detect press-release datelines in Exhibit 99.1.';

alter table public.world_cities enable row level security;

-- WORLD_CITIES_DATA (GeoNames cities, population >= 15,000)
insert into public.world_cities (ascii_name, name)
select x, x
from unnest(string_to_array($geonames_cities$
'ABASAN AL KABIRAH
'AFAK
'AIN ABID
'AIN AZEL
...
ORTIGUEIRA
ORUMIYEH
ORUNIA GORNA-GDANSK PO UDNIE
ORUNIA GORNA-GDANSK POLUDNIE
ORURO
ORVAULT
ORZESZE
OSAKA
...
PILAR DO SUL
PILCZYCE-KOZANOW-POPOWICE PO NOCNE
...
$geonames_cities$, E'\n')) as x
where length(x) >= 4
on conflict (ascii_name) do nothing;
