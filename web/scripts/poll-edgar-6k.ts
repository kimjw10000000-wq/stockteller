/**
 * VPS loop: SEC current 8-K and 6-K Atom → Exhibit 99.1 press releases → wire_news.
 * Weekday 04:00–20:00 ET (pre/regular/after): every 200ms, one Atom per tick (8-K and 6-K alternate).
...
const FORMS: EdgarPressForm[] = ["8-k", "6-k"];
...
async function loop(): Promise<void> {
  ...
  for (;;) {
    ...
    const form = FORMS[tick++ % FORMS.length]!;
    try {
      ...
      console.error(new Date().toISOString(), form, e instanceof Error ? e.message : e);
    }
